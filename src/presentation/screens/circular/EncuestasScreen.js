...
const EncuestasScreen = ({ navigation }) => {
  ...
  const fetchEncuestas = async () => {
    try {
      setLoading(true);
      const data = await circularRepository.getNoticesSurveys();

      if (data.code === 200 && data.response) {
        const list = Object.keys(data.response)
          .filter(key => key !== 'keys')
  ...
  const getFiltered = () => {
    switch (filterTab) {
      case 'Vista': return items.filter(c => c.state === '1');
      case 'Pendiente': return items.filter(c => c.state !== '1');
      case 'Autorizado': return items.filter(c => c.type === '1' && (c.auth || '').toLowerCase().trim() === 'si');
      case 'No autorizado': return items.filter(c => c.type === '1' && (c.auth || '').toLowerCase().trim() === 'no');
        ...
        <View style={styles.cardsContainer}>
          {filtered.length === 0 ? (
            <View style={styles.emptyContainer}>
              <Icon name="clipboard-list-outline" size={64} color="#cbd5e1" />
              <Text style={styles.noItemsText}>No hay encuestas disponibles en este momento.</Text>
            </View>
          ) : (
            filtered.map((circular, index) => {
              ...
              return (
                <TouchableOpacity
                  key={`encuesta-${index}-${circular.circular}`}
                  style={styles.circularCard}
                  onPress={() =>
                    navigation.navigate('DetalleEncuesta', {
                      num_survey: circular.circular,
                    })
                  }
                  activeOpacity={0.7}
                >
                  <View style={styles.iconContainer}>
                    <Icon name="clipboard-list-outline" size={28} color="#002c5d" />
                  </View>
...
const styles = StyleSheet.create({
  ...
  noItemsText: { color: '#64748b', textAlign: 'center', fontSize: 16, marginTop: 16 },

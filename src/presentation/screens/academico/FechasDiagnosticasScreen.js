...
const parseFecha = (fechaStr) => {
  ...
  // Intentar formato " Tuesday 03 February 26" o similar
...
const FechasDiagnosticasScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        setError(null);
        const result = await fechasDiagnosticasRepository.getEvaluacionesDiagnosticas('2');
        ...
        console.error('Error al cargar fechas diagnósticas:', err);
  ...
  return (
    <AppScreenLayout>
      ...
      >
        ...
        {/* Evaluations List - Timeline Style */}
        {evaluations.length === 0 ? (
          <View style={styles.emptyContainer}>
            <Icon name="calendar-blank-outline" size={48} color="#94a3b8" />
            <Text style={styles.emptyTitle}>Sin datos</Text>
            <Text style={styles.emptyText}>
              No hay evaluaciones diagnósticas para el periodo seleccionado.
            </Text>
          </View>
        ) : (
